...
def compute_profit_by_pair(trades_to_order_by_pair):
    ...
    for arbitrage_id in orders_by_arbitrage_id:
        if len(orders_by_arbitrage_id[arbitrage_id]) > 2:
...
def compute_loss(orders_by_arbitrage_id):
    ...
    for arbitrage_id in orders_by_arbitrage_id:
        if len(orders_by_arbitrage_id[arbitrage_id]) != 1:
            continue
        order, trades_list = orders_by_arbitrage_id[arbitrage_id]
    ...
    for order, trade_list in orders_and_trades_by_pair:
        if order.trade_type == DEAL_TYPE.BUY:
            for trade in trade_list:
                ...
                volume_bitcoin -= trade.volume * trade.price
...
if __name__ == "__main__":
    ...
    for pair_id in loss_by_coin_bitcoin:
        pair_name = get_pair_name_by_id(pair_id)
        log_to_file("{pn} - Volume change {p} Bitcoin volume change {vp}".format(pn=pair_name,
                                                                                 p=float_to_str(loss_by_coin[pair_id]),
                                                                                 vp=float_to_str(loss_by_coin_bitcoin[pair_id])),
                    "what_we_have_at_the_end.log")